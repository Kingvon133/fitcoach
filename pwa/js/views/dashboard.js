...
export function renderDashboard(container) {
  ...
  container.innerHTML = `
    ...
    <div class="card">
      <div class="kcal-row">
        <div class="ring-wrap">
          ${progressRing(progress, { colorKey: over ? 'orange' : 'green' })}
        ...
        <div class="kcal-info">
          <span><span class="inline-icon accent-purple">${icon('target')}</span>Obiettivo <b>${targets.kcal}</b></span>
          <span><span class="inline-icon accent-orange">${icon('flame')}</span>Rimanenti <b>${Math.max(targets.kcal - Math.round(kcal), 0)}</b></span>
          ${over ? `<span style="color:var(--orange)">+${Math.round(kcal - targets.kcal)} oltre l'obiettivo</span>` : ''}
        </div>
      </div>
    </div>

    <div class="macro-row">
      ${macroCell('Proteine', protein, targets.protein, 'm-protein', 'var(--blue)')}
      ${macroCell('Carboidrati', carbs, targets.carbs, 'm-carbs', 'var(--orange)')}
      ${macroCell('Grassi', fat, targets.fat, 'm-fat', 'var(--purple)')}
    </div>

    ${todayDay ? `
      <div class="card mt16">
        <div class="card-header">
          <span><span class="inline-icon accent-blue">${icon('dumbbell')}</span>Allenamento di oggi</span>
    ...
    ${weights.length >= 2 ? `
      <div class="card">
        <div class="card-header">
          <span><span class="inline-icon accent-purple">${icon('scale')}</span>Peso</span>
          <span class="hint">${weights[weights.length - 1].kg.toFixed(1)} kg · ${formatDateShort(weights[weights.length - 1].date)}</span>
        </div>
        ${lineChart(weights.map(w => ({ label: w.date, value: w.kg })), { colorKey: 'purple' })}
      </div>` : `
      <div class="card">
...
function macroCell(name, value, target, cellClass, barColor) {
  const progress = target > 0 ? Math.min(value / target, 1) : 0;
  return `<div class="macro-cell ${cellClass}">
    <div class="macro-name">${name}</div>
    <div class="macro-val">${Math.round(value)}g</div>
    <div class="macro-target">/ ${target}g</div>
    <div class="macro-bar"><div style="width:${(progress * 100).toFixed(0)}%;background:${barColor}"></div></div>
  </div>`;
}
